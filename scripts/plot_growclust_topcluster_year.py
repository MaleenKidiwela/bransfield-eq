"""Zoom in on the largest GrowClust cluster in the FULL-YEAR catalog. Companion
to plot_growclust_topcluster.py (which targets the 30-day catalog). Writes to a
separate filename so the 30-day figure is preserved unchanged."""
...
OUT = REPO / "notes" / "figures" / "growclust_topcluster_year.png"
...
# Time series: events per day across the cluster span. Year span -> 1-day bins.
ax = fig.add_subplot(gs[1, 1])
import matplotlib.dates as mdates
ax.hist(clu.dt, bins=80, color="steelblue", edgecolor="k", linewidth=0.3)
span_days = (clu.dt.max() - clu.dt.min()).days
if span_days > 60:
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=max(1, span_days // 240)))
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))
else:
    ax.xaxis.set_major_locator(mdates.DayLocator(interval=max(1, span_days // 10)))
...
fig.suptitle(f"Largest GrowClust cluster (year-long catalog) — {len(clu)} relocated events, "
