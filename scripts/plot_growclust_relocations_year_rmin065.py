"""Year-long GrowClust relocations on Orca. Companion to plot_growclust_relocations.py
(which targets the 30-day partial catalog). Writes a separate figure so the
30-day plot is preserved unchanged."""
...
GROW_CSV = REPO / "catalogs" / "growclust_picker_only_rmin065.csv"
ST = REPO / "catalogs" / "station_geometry.csv"
BATHY = REPO / "notes" / "figures" / "Orca_bathymetry.nc"
OUT = REPO / "notes" / "figures" / "growclust_relocations_year_rmin065.png"
...
df = pd.read_csv(GROW_CSV, names=COLS)
moved = ((df.lat_gc != df.lat_py) | (df.lon_gc != df.lon_py) |
         (df.dep_gc != df.dep_py))
...
def plot_bathy(ax):
    ...
    cf = ax.contourf(LON, LAT, z_b, levels=levels, cmap=cmap, norm=norm, extend="both")
...
fig.suptitle(f"GrowClust relocations on the full-year catalog (rmin=0.65)\n"
