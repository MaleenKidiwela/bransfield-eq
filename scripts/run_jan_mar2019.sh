#!/bin/bash
# Run XC prep (legacy eager-preload path) + GrowClust on the Jan 1 - Mar 15
# 2019 subset to demonstrate that the original front-load approach scales to
# the maximum date window allowed by the pod (~80 days).
set -e
cd /home/jovyan/bransfield-eq

LABEL="jan_mar2019"
LOG="logs/run_${LABEL}.log"
...
log "=== launching XC prep (front-load, --preload) for $LABEL ==="
...
stlist.write_text("\n".join(out) + "\n")
print(f"  rewrote stlist with {len(out)} bare codes")
