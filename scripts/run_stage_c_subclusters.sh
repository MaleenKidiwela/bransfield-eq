...
REGIONS_CSV="catalogs/stage_c_regions.csv"
LABELS=$(.venv/bin/python -c "
import pandas as pd
r = pd.read_csv('$REGIONS_CSV')
print(' '.join(r.label.tolist()))
")
echo "Stage C labels: $LABELS"
export OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 NUMEXPR_NUM_THREADS=1
run_one () {
    local LABEL="$1"
    local LOG="logs/stage_c_${LABEL}.log"
    {
...
PIDS=()
for L in $LABELS; do run_one "$L" & PIDS+=($!); done
echo "Launched ${#PIDS[@]} parallel pipelines."
for p in "${PIDS[@]}"; do wait "$p"; done
echo "All Stage C pipelines done."
