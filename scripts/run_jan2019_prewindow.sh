#!/bin/bash
# Jan 1 -> Jan 30 2019 subset, pre-windowing pipeline. Output matches the
# original 30-day partial catalog scope so the relocations are directly
# comparable to growclust_partial30days.csv / growclust_relocations.png.
set -e
...
log "=== done: catalogs/growclust_${LABEL}.csv ($(wc -l < catalogs/growclust_${LABEL}.csv) rows) ==="
